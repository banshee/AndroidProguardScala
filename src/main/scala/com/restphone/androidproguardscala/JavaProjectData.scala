...
class JavaProjectData(project: IJavaProject) {
  ...
  val preferences = {
    ...
    for {
      ...
      defaultValue = defaultValueForClasspathEntry(entry)
    } yield {
  ...
  def defaultValueForClasspathEntry(c: IClasspathEntry): ClasspathEntryType = {
    val MatchesAndroidSdk = ".*android-sdk.*".r
    val AndroidSupport = """android-support-v\d+.jar""".r
    val Scalaz = """scalaz-.*jar""".r
    val Akka = """akka-.*jar""".r
    val (a, b) = (c.getPath.toOSString, c.getPath.lastSegment)
